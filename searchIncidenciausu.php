...
<body>
	<div class="container">
		...
		<?php 
            $id = $_GET["id"];
            $sentencia = $mysqli->prepare("SELECT idInc, data, prioritat, descripcio, tipus, tecnic, departament FROM INCIDENCIA WHERE idInc = ?");
			...
			if ($incidencia) {
			    ...
			    header("Location: errorIncidenciaUsu.html");
			    echo '<br> error';
			}
		?>
		<button onclick="location.href='./cercarIncidenciausu.php'">Volver</button>
		<button onclick="location.href='./listarActuacioUsu.php?idInc=<?php echo $incidencia['idInc'] ?>'">Mostrar Actuaciones</button>
	</div>
</body>
</html>
